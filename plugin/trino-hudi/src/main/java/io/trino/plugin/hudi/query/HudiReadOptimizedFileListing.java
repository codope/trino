        ...
        extends HudiFileListing
{
    ...
    public List<HudiPartitionInfo> getPartitionsToScan()
    {
        ...
        if (shouldSkipMetastoreForPartition) {
            try {
                ...
                LOG.warn("Cannot skip Hive Metastore for scanning partitions.  "
                        + "Falling back to using Hive Metastore ...");
            }
        }
    ...
    @VisibleForTesting
    PartitionValueExtractor inferPartitionValueExtractorWithHiveMetastore() throws HoodieIOException
    {
        hivePartitionNames = getPartitionNamesFromHiveMetastore(TupleDomain.all());
        if (hivePartitionNames.isEmpty()) {
            throw new HoodieIOException("Cannot infer partition value extractor "
                    + "with Hive Metastore: partition list is empty!");
        }
        HudiPartitionHiveInfo partitionHiveInfo = new HudiPartitionHiveInfo(
                ...
                tableHandle.getPartitionPredicates(), hiveTable, hiveMetastore, hiveIdentity);
        ...
        List<String> partitionValues = partitionHiveInfo.getHivePartitionKeys().stream()
                .map(key -> key.getValue()).collect(Collectors.toList());
